...
Load `references/skill-quality-principles.md` first; it is the bar. Cite its rules in findings rather than restating them.
...
- **File topology** matches the carve-out rule: content stays inline until SKILL.md is too large to scan, then moves to `references/` with descriptive names, one level deep. Flag `*.md` workflow content sitting at skill root, and any SKILL-to-reference-to-reference nesting.
