...
2. Resume check. Look for an existing `.memlog.md` in the run folder. If one is found, read it once to rebuild state and continue append-only; otherwise initialize a new memlog with `python3 {project-root}/_bmad/scripts/memlog.py init --path <run-folder>/.memlog.md`.
